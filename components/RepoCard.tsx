...
export function RepoCard({ repo, index }: { repo: Repo; index: number }) {
  ...
  return (
    ...
    >
      <div className="grain relative h-full bg-white/65 hover:bg-white/80 backdrop-blur-lg backdrop-saturate-150 border border-white/50 ring-1 ring-slate-200/30 hover:ring-slate-300/40 rounded-3xl overflow-hidden transition-all duration-200 shadow-[inset_0_1px_0_rgba(255,255,255,0.6)] hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.6),0_8px_24px_-16px_rgba(15,23,42,0.1)]">
        <div className="pointer-events-none absolute -inset-px rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-br from-slate-200/30 via-transparent to-transparent" />
        ...
        <div className="relative p-5 flex flex-col h-[calc(100%-45px)]">
          ...
          <p className="text-sm leading-relaxed text-slate-500 mb-5 flex-1">{repo.desc}</p>
          ...
          <div className="flex items-center justify-between mt-auto pt-3.5 border-t border-slate-200/50">
            ...
            <div className="flex items-center gap-1.5 text-xs font-bold text-slate-400 group-hover:text-slate-800 transition-colors duration-200">
