...
export function FeaturedProject({ repo }: { repo: Repo }) {
  return (
    <motion.div variants={fadeUp} transition={spring} className="relative">
      <div className="grain relative overflow-hidden rounded-[2rem] border border-white/50 ring-1 ring-slate-200/30 bg-gradient-to-br from-amber-50/80 via-white/65 to-emerald-50/80 backdrop-blur-lg backdrop-saturate-150 shadow-[inset_0_1px_0_rgba(255,255,255,0.6)]">
        ...
        <div className="relative grid md:grid-cols-[1.2fr_1fr] gap-6 md:gap-8 p-8 md:p-10">
          <div className="flex flex-col">
            ...
            <h3 className="text-3xl md:text-5xl font-black tracking-tight text-slate-900 leading-[0.95]">
              {repo.displayName}
            </h3>
            ...
            <p className="mt-5 text-base md:text-lg text-slate-500 leading-relaxed max-w-xl">
              {repo.desc}
            </p>
              ...
              <div className="p-5 font-mono text-[11px] md:text-xs space-y-2 text-white/80">
                ...
                <div className="pt-3 grid grid-cols-3 gap-2">
                  {["AI", "TS", "Next", "Edge", "RSS", "NLP"].map((t) => (
                    <span key={t}
                      className="text-center px-2 py-1.5 rounded-lg bg-white/[0.06] border border-white/[0.12] text-white/70 text-[10px] font-bold tracking-[0.15em] shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] hover:bg-white/[0.1] transition-colors duration-200">
                      {t}
                    </span>
                  ))}
                </div>
